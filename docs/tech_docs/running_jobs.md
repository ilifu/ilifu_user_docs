...
The slurm documentation [on priority](https://slurm.schedmd.com/priority_multifactor.html) and [fairshare](https://slurm.schedmd.com/fair_tree.html) is very extensive, what follows here is a short summary of the principals behind priority and fairshare; and how to check what's going on with your jobs on ilifu.
...
3. **Age**: The age factor is intentionally set very small so that as jobs age their priority increases slightly. This is so that the _FairShare_ factor is dominant in most cases.
